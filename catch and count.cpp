    ...
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            ...
            if (i == 21 || i == 22 || i == 23) {
                ...
                // Custom color for row 25
                buffer[i * cols + j].Attributes = 8000;
            }
            else {
                // Default section with white background and black foreground
...
void drawNumbers(CHAR_INFO buffer[]) {
    ...
    for (auto number : fallingNumbers) {
        buffer[number.first * cols + number.second].Char.AsciiChar = (char)(number.second % 10 + '0');  // Use stored number value, modulo by 10 to ensure single digits
